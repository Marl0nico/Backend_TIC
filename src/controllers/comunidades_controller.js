...
const unirseComunidad = async (req, res) => {
  try {
    // console.log('-- unirseComunidad --');
    // console.log('req.headers.authorization:', req.headers.authorization);
    // console.log('req.usuario:', req.usuario);
    // console.log('req.body:', req.body);
    const comunidadId = req.params.id;
    const estudianteId = req.usuario._id; // mejor desde el token
    ...
    // Actualizar ambos con $addToSet (evita duplicados)
    ...
    await Estudiante.findByIdAndUpdate(
      estudianteId,
      { $addToSet: { comunidades: comunidadId } }
    );

    res.status(200).json({ mensaje: "Te has unido a la comunidad correctamente" });

  } catch (error) {
    console.error("Error unirse a comunidad:", error);
    res.status(500).json({
      mensaje: "Hubo un error al unirse a la comunidad",

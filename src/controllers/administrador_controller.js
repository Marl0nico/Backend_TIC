...
const perfil = async (req, res) => {
  ...
  if (rol !== "Administrador")
    return res.status(404).json({ msg: "No tienes permisos para realizar esta acción" });
...
const actualizarPassword = async (req, res) => {
  ...
  const administradorBDD=await Administrador.findById(req.body._id);
  if (!administradorBDD){
    return res.status(404).json({msg: "No existe el administrador"});
  }
  const verificarPassword=await administradorBDD.matchPassword(req.body.passwordactual);
  if (!verificarPassword){
    return res.status(404).json({msg: "La contraseña actual es incorrecta"});
  }
  administradorBDD.password=await administradorBDD.encrypPassword(req.body.passwordnuevo);
  await administradorBDD.save();
  res.status(200).json({msg: "Contraseña actualizada conrrectamente"});
};
